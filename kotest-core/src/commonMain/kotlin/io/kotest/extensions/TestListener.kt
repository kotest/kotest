...
interface TestListener {
    ...
    */
   @Deprecated(
      "Prefer using ProjectListeners over TestListener for project before/after",
      ReplaceWith("ProjectListener")
   )
    ...
    *
    * @param spec the [SpecInterface] instance.
    */
   @Deprecated("to be adapted")
   fun beforeSpec(spec: SpecInterface): Unit = Unit

   /**
    * Is invoked after the [TestCase]s that are part of a particular
    * [SpecInterface] instance have completed.
    *
    * If a spec is instantiated multiple times - for example, if
    ...
    */
   @Deprecated("to be adapted")
   fun afterSpec(spec: SpecInterface): Unit = Unit

   /**
    * Called once per [SpecInterface], when the engine is preparing to
    ...
    */
   fun beforeSpecClass(spec: SpecInterface, tests: List<TopLevelTest>): Unit = Unit

   /**
    * Called once per [SpecInterface], after all tests have completed for that spec.
    *
    * Regardless of how many times the spec is instantiated,
    * for example, if [InstancePerTest] or [InstancePerLeaf] isolation
    * modes are used, this callback will only be invoked once,
    ...
    */
   fun afterSpecClass(spec: SpecInterface, results: Map<TestCase, TestResult>): Unit = Unit

   /**
    * Is invoked after all the [SpecInterface] classes have been discovered.
   ...
   /**
    * Is invoked each time a [SpecInterface] is started.
    *
    ...
    * @param spec the actual [SpecInterface] instance.
    */
   @Deprecated("use beforeSpec(Spec)", ReplaceWith("beforeSpec(Spec)"))
   fun beforeSpec(description: Description, spec: SpecInterface): Unit = Unit

   /**
    * Is invoked each time a [SpecInterface] completes.
    *
    * Note: If the spec is running with one instance per test, then this
    * function will be invoked multiple times.
    *
   ...
   fun afterSpec(description: Description, spec: SpecInterface): Unit = Unit
    ...
    */
   @Deprecated(
      "use beforeTest(TestCase) which provides the full test case instance",
      ReplaceWith("beforeTest(TestCase)")
   )
...
data class RootTests(val tests: List<TopLevelTest>)
