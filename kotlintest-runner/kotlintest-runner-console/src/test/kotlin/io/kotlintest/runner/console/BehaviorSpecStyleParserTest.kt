...
class BehaviorSpecStyleParserTest : FunSpec() {
  ...
  init {
    ...
    test("should parse given when") {
      BehaviorSpecStyleParser.parse(Description.spec("myspec"), "Given: foo When: bar") shouldBe
    ...
    test("should parse given when then") {
      BehaviorSpecStyleParser.parse(Description.spec("myspec"), "Given: foo When: bar   Then: waz") shouldBe
          Description.spec("myspec").append("Given: foo").append("When: bar  ").append("Then: waz")

      BehaviorSpecStyleParser.parse(Description.spec("myspec"), "Given: foo234 When: bar-- Then: waz$$  ") shouldBe
