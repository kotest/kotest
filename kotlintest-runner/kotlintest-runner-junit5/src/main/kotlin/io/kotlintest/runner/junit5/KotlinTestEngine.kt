...
class KotlinTestEngine : TestEngine {
  ...
  private fun runTest(descriptor: TestCaseDescriptor, listener: EngineExecutionListener) {
    ...
    val extensions: List<TestCaseExtension> = descriptor.testCase.config.extensions + descriptor.testCase.spec.testCaseExtensions() + Project.testCaseExtensions()
    ...
    chain {
      ...
      when (result.status) {
        ...
        TestStatus.Error -> listener.executionFinished(descriptor, TestExecutionResult.aborted(result.error))
