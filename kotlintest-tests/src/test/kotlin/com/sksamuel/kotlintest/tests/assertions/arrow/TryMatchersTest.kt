...
import io.kotlintest.assertions.arrow.`try`.failure
import io.kotlintest.assertions.arrow.`try`.success
  ...
  init {
    ...
    "Try shouldBe Success(value)" should {
      "test that a try is a Success with the given value" {
        ...
        shouldThrow<AssertionError> {
          Try.Failure<Nothing>(RuntimeException()) shouldBe success("foo")
        }.message shouldBe "Try should be a Success(foo) but was Failure(null)"

        shouldThrow<AssertionError> {
          Try.Success("boo") shouldBe success("foo")
    ...
    "Try shouldBe Failure" should {
      "test that a try is a Failure" {
        shouldThrow<AssertionError> {
          Try.Success("foo") shouldBe failure()
