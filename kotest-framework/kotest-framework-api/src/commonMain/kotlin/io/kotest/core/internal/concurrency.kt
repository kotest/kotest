 ...
 *
 * Note that if this spec is annotated @Isolate then the value will be [ConcurrencyMode.Isolated]
 * regardless of the config setting.
 */
fun Spec.resolvedConcurrencyMode(): ConcurrencyMode =
   if (this::class.isIsolate()) ConcurrencyMode.Isolated else
      this.concurrencyMode() ?: configuration.concurrencyMode ?: defaultConcurrencyMode()

internal fun defaultConcurrencyMode() =
   if (configuration.parallelism > 1) ConcurrencyMode.Concurrent else ConcurrencyMode.Isolated
