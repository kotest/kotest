...
interface ShouldSpecRootContext : RootContext {
   ...
   fun should(name: String) =
      RootTestWithConfigBuilder(createTestName(name, "should ", null, true), registration(), xdisabled = false)

   fun xshould(name: String) =
      RootTestWithConfigBuilder(createTestName(name, "should ", null, true), registration(), xdisabled = true)
   ...
   fun should(name: String, test: suspend TestContext.() -> Unit) =
      registration().addTest(createTestName(name, "should ", null, true), xdisabled = false, test = test)

   fun xshould(name: String, test: suspend TestContext.() -> Unit) =
      registration().addTest(createTestName(name, "should ", null, true), xdisabled = true, test = test)
