...
data class TestName(
   ...
   companion object {
      ...
      operator fun invoke(name: String): TestName = TestName(name, null, null, false)
      ...
      operator fun invoke(
         ...
         val (focus, bang, croppedName) = when {
            name.startsWith("!") -> Triple(first = false, second = true, third = name.drop(1).trim())
            name.startsWith("f:") -> Triple(first = true, second = false, third = name.drop(2).trim())
            else -> Triple(first = false, second = false, third = name)
         }
