...
class TestCaseExecutionListenerToTestEngineListenerAdapter(
   ...
   override suspend fun testFinished(testCase: TestCase, result: TestResult) {
      logger.log { Pair(testCase.name.name, "Adapting testFinished to engine event $result") }
      listener.testFinished(testCase, result)
   }

   override suspend fun testIgnored(testCase: TestCase, reason: String?) {
      logger.log { Pair(testCase.name.name, "Adapting testIgnored to engine event $reason") }
      listener.testIgnored(testCase, reason)
   }

   override suspend fun testStarted(testCase: TestCase) {
      logger.log { Pair(testCase.name.name, "Adapting testStarted to engine event") }
