...
class TestEngine(initial: TestEngineConfig) {
   ...
   suspend fun execute(suite: TestSuite): EngineResult {
      ...
      val execute: suspend (EngineContext) -> EngineResult =
         config.interceptors.foldRight(innerExecute) { extension, next ->
      ...
      return coroutineScope {
         execute(EngineContext(config.listener, suite, config.tags, config.configuration))
      }
   }
}
