...
class ExpectSpecIncompleteContainerTest : FunSpec() {
   init {
      test("expect spec should error if not complete") {
         val collector = CollectingTestEngineListener()
         TestEngineLauncher(collector)
            .withClasses(IncompleteExpectSpec::class)
            .launch()
...
private class IncompleteExpectSpec : ExpectSpec() {
   init {
      context("a") { }
      context("b") {
         context("c") {}
