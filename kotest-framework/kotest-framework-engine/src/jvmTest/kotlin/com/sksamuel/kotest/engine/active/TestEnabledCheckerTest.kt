...
@ExperimentalKotest
class TestEnabledCheckerTest : StringSpec() {

   init {

      "TestEnabledChecker should return false if the test is disabled in config" {
         val test = TestCase(
            name = TestNameBuilder.builder("foo").build(),
            descriptor = TestEnabledCheckerTest::class.toDescriptor().append("foo"),
            spec = this@TestEnabledCheckerTest,
            parent = null,
            test = {},
         ...
         TestEnabledChecker(
            ...
            SpecConfigResolver(),
            TestConfigResolver()
         ).isEnabledInternal(test).isEnabled shouldBe false
      }

      "TestEnabledChecker should return false if it has an excluded tag" {
      ...
      "TestEnabledChecker should return false if it is excluded by a tag expression" {
         val mytag = NamedTag("mytag")
      ...
      "isEnabledInternal should return true if the test is focused and top level" {
         ...
         TestEnabledChecker(
            ...
            SpecConfigResolver(),
            TestConfigResolver()
         ).isEnabledInternal(test).isEnabled shouldBe true
      }
      ...
      "isEnabledInternal should return false if a test filter excludes the test" {
         ...
         val filter = object : DescriptorFilter {
            override fun filter(descriptor: Descriptor): DescriptorFilterResult {
               return if (descriptor.id.value == "f") DescriptorFilterResult.Include else DescriptorFilterResult.Exclude(
                  null
               )
            }
         }
         ...
         checker.isEnabledInternal(tc1).isEnabled shouldBe true
         ...
         checker.isEnabledInternal(tc2).isEnabled shouldBe false
      }
      ...
      "isEnabled should use extensions when registered" {
         ...
         checker.isEnabledInternal(tc2).isEnabled shouldBe true
      }
   }
}
