...
class IsolatedReporter(val reporter: Reporter) : Reporter {
   ...
   private val runningSpec = AtomicReference<Descriptor.SpecDescriptor>(null)
   ...
   override fun specStarted(kclass: KClass<*>) {
      synchronized(reporter) {
         if (runningSpec.compareAndSet(null, Descriptor.SpecDescriptor(kclass))) {
   ...
   override fun specFinished(kclass: KClass<*>, t: Throwable?, results: Map<TestCase, TestResult>) {
      synchronized(reporter) {
         if (runningSpec.get() == Descriptor.SpecDescriptor(kclass)) {
