...
expect fun KClass<*>.qualifiedSpecName(): String
...
data class Description(val parents: List<String>, val name: String) {
  ...
  companion object {
    fun spec(klass: KClass<out Spec>) = spec(klass.qualifiedSpecName())
  ...
  fun parent(): Description? = if (isSpec()) null else Description(parents.dropLast(1),
      parents.last())
  ...
  fun tail() = if (parents.isEmpty()) throw NoSuchElementException() else Description(parents.drop(1),
      name)
