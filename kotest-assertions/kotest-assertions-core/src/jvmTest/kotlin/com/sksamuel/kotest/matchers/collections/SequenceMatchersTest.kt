   ...
   init {
      ...
      "larger than" should {
         ...
         fail("for constrained for same count") {
            countup.constrainOnce().shouldBeLargerThan(countdown)
         }

         succeed("for constrained for larger") {
            countup.constrainOnce().shouldBeLargerThan(nulls)
         }
      }
      ...
      "at most count" should {
         ...
         fail("for constrained for smaller count") {
            countup.constrainOnce().shouldHaveAtMostCount(countup.count() - 1)
         }

         succeed("for constrained for same count") {
            countup.constrainOnce().shouldHaveAtMostCount(countup.count())
         }

         succeed("for constrained for larger count") {
            countup.constrainOnce().shouldHaveAtMostCount(countup.count() + 1)
         }
      }
      ...
      "contain only nulls" should {
         ...
         succeed("for nulls constrained") {
            nulls.constrainOnce().shouldContainOnlyNulls()
         }

         fail("for sparse constrained") {
            sparse.constrainOnce().shouldContainOnlyNulls()
         }
      }
      ...
      "not contain only nulls" should {
         ...
         fail("for nulls constrained") {
            nulls.constrainOnce().shouldNotContainOnlyNulls()
         }

         succeed("for sparse constrained") {
            sparse.constrainOnce().shouldNotContainOnlyNulls()
         }
      }
      ...
      "contain a null" should {
         ...
         fail("for non-nulls constrained") {
            single.constrainOnce().shouldContainNull()
         }

         succeed("for sparse constrained") {
            sparse.constrainOnce().shouldContainNull()
         }
      }
      ...
      "not contain a null" should {
         ...
         succeed("for non-nulls constrained") {
            single.constrainOnce().shouldNotContainNull()
         }

         fail("for sparse constrained") {
            sparse.constrainOnce().shouldNotContainNull()
         }
      }
      ...
      "have a lower bound" should {
         ...
         succeed("for constrained for multiple") {
            countup.constrainOnce().shouldHaveLowerBound(countup.minOrNull() ?: Int.MIN_VALUE)
         }

         fail("for constrained for multiple with wrong bound") {
            countup.constrainOnce().shouldHaveLowerBound((countup.minOrNull() ?: Int.MIN_VALUE) + 1)
         }
      }
