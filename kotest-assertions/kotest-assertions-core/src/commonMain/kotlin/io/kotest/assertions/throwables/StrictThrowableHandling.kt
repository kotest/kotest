...
inline fun <reified T : Throwable> shouldThrowExactly(block: () -> Any?): T {
   ...
   return when {
      thrownThrowable == null -> AssertionErrorBuilder.fail("Expected exception ${T::class.bestName()} but no exception was thrown.")
...
inline fun <reified T : Throwable> shouldNotThrowExactly(block: () -> Any?) {
   ...
   if (thrown::class == T::class) throw AssertionErrorBuilder.create()
      .withMessage("No exception expected, but a ${thrown::class.simpleName} was thrown.")
      .withCause(thrown)
      .build()
   throw thrown
}
