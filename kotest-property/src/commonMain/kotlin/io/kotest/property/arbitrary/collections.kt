...
/**
 * Returns an [Arb] whose of values are a set of values generated by the given arb.
 * The size of each set is determined randomly up to the specified [max].
 */
@JvmOverloads
fun <T> Gen.Companion.set(arb: Arb<T>, maxSize: Int = 100): Arb<Set<T>> = object : Arb<Set<T>> {

   init {
      require(maxSize >= 0) { "maxSize must be positive" }
   }

   override fun edgecases(): List<Set<T>> = listOf(arb.edgecases().take(maxSize).toSet())
   override fun sample(rs: RandomSource): Sample<Set<T>> {
      val size = rs.random.nextInt(maxSize)
      val set = generateSequence { arb.sample(rs) }.map { it.value }.take(size).toList().toSet()
      return Sample(set)
   }
}
