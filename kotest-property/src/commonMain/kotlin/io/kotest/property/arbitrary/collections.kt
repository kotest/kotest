 ...
 */
@JvmOverloads
fun <A> Arb.Companion.list(gen: Gen<A>, range: IntRange = PropertyTesting.defaultCollectionsRange): Arb<List<A>> {
   check(!range.isEmpty())
   check(range.first >= 0)
...
/**
 * Returns an [Arb] which returns lists that contain values generated by the given generator [gen].
 * The size of each list is determined randomly within the specified [range].
 *
 * The edge cases are the empty list, if the range includes zero, and a list with a repeated element (taken
 * as the first edge case from the underlying gen).
