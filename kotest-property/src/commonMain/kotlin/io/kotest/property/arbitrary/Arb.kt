...
fun <A, B> Arb<A>.distinctBy(selector: (A) -> B) = object : Arb<A> {
   ...
   override fun samples(rs: RandomSource): Sequence<Sample<A>> {
      return this@distinctBy.samples(rs).distinctBy { selector(it.value) }
   }
   override fun generate(rs: RandomSource): Sequence<Sample<A>> {
      return (this@distinctBy.edgecases().asSequence().map { Sample(it) } + this@distinctBy.samples(rs)).distinctBy { selector(it.value) }
   }
}
