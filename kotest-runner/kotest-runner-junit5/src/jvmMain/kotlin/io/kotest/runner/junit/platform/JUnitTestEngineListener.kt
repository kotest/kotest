...
class JUnitTestEngineListener(
   ...
   // contains a mapping of a TestId to a junit TestDescription, so we can look up the parent
   ...
   override fun specStarted(kclass: KClass<out Spec>) {
      ...
      try {
         ...
         descriptorsByTestId[kclass.toDescription().toDescriptor(sourceRef()).id()] = descriptor
   ...
   override fun specFinished(
      ...
      val descriptor = descriptorsByTestId[kclass.toDescription().toDescriptor(sourceRef()).id()]
   ...
   private fun ensureSpecIsVisible(kclass: KClass<out Spec>, t: Throwable) {
      if (!hasVisibleTest) {
         ...
         val spec = descriptorsByTestId[description.toDescriptor(sourceRef()).id()]!!
   ...
   override fun testFinished(testCase: TestCase, result: TestResult) {
      val descriptor = descriptorsByTestId[testCase.description.toDescriptor(testCase.source).id()]
   ...
   override fun testFinished(descriptor: Descriptor.TestDescriptor, result: TestResult) {
      val descriptor = descriptorsByTestId[descriptor.id()]
   ...
   private fun createTestDescriptor(testCase: TestCase): TestDescriptor {
      val parent = descriptorsByTestId[testCase.description.parent.toDescriptor(testCase.source).id()]
      if (parent == null) {
      ...
      descriptorsByTestId[testCase.description.toDescriptor(testCase.source).id()] = descriptor
      return descriptor
   }

   private fun createTestDescriptor(descriptor: Descriptor.TestDescriptor): TestDescriptor {
      val parent = descriptorsByTestId[descriptor.parent.id()]
      if (parent == null) {
      ...
      descriptorsByTestId[descriptor.id()] = td
      return td
   }
