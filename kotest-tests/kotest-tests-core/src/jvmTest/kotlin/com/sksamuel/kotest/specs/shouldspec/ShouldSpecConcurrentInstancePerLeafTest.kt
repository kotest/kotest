...
class ShouldSpecConcurrentInstancePerLeafTest : ShouldSpec() {
   ...
   init {
      ...
      context("picard") {
         synchronized(Counters) { Counters.executed.add(this.testContext.testCase.descriptor.name.name) }
         synchronized(Counters) { Counters.threads.add(Thread.currentThread().id) }
         context("riker") {
            synchronized(Counters) { Counters.executed.add(this.testContext.testCase.descriptor.name.name) }
            synchronized(Counters) { Counters.threads.add(Thread.currentThread().id) }
            should("data") {
               synchronized(Counters) { Counters.executed.add(this.testCase.descriptor.name.name) }
               synchronized(Counters) { Counters.threads.add(Thread.currentThread().id) }
               delay(1000)
            }
            xshould("geordi") {
