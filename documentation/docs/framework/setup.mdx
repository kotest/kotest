...
The Kotest test framework is supported on all targets, including JVM, JavaScript, Native (Linux, MacOS, watchOS, iOS, tvOS, Windows) and Wasm.
...
:::caution[Migrating to Kotest 6]
The Kotlin multiplatform setup in Kotest 6.0 has changed from previous versions. The Kotest Gradle plugin has been
renamed to `io.kotest`. Also, the compiler plugin has been replaced with a more robust method using KSP.
...
Kotest on the JVM builds atop of the [JUnit
Platform](https://junit.org/junit5/docs/current/user-guide/#running-tests-build-gradle) project which is widely
supported in the JVM ecosystem.
...
Note, place the KSP plugin before the Kotest plugin in the plugins block. This restriction will be fixed in a future release.
...
:::tip[Example Project]
A working native project with Linux, Windows and MacOS targets configured, with unit and data driven test examples, can be found here:
https://github.com/kotest/kotest-examples
:::
...
Kotest supports both Unit tests and Instrumented tests on Android.

:::tip[Example Project]
An example Android project with unit, instrumented and data-driven tests can be found here:
https://github.com/kotest/kotest-examples/tree/master/kotest-android
:::

## Unit Tests

Android Unit tests build on top of [JUnit Platform](https://junit.org/junit5/docs/current/user-guide/#running-tests-build-gradle) like regular JVM tests.
This requires configuring the android test options block in your build file and then adding the Kotest JUnit5 runner dependency to the `testImplementation` sourceset.
These tests typically reside inside of `src/test/kotlin`.
...
Android Instrumented tests build on top of JUnit4 like the default Android instrumented tests.
This requires adding the Kotest JUnit4 runner dependency to the `androidTestImplementation` sourceset.
These tests typically reside inside of `src/androidTest/kotlin`.
...
Then you must annotate your test classes with `@RunWith(KotestTestRunner::class)`

For example:

```kotlin
@RunWith(KotestTestRunner::class)
class MyInstrumentedTest : ShouldSpec() {
   init {
...
:::note
The JS, Wasm and Native test engines are feature limited compared to the JVM test engine. The major restriction is that annotation
...
The plugin is available in the [Jetbrains marketplace](https://plugins.jetbrains.com/plugin/14080-kotest) and can be installed
from the IDE by going to Preferences -> Plugins -> Marketplace -> Search for Kotest.

For more details, see the [Intellij Plugin page](https://kotest.io/docs/intellij/intellij-plugin.html).

### Enhanced IDE support

For enhanced support such as jump-to-source and re-running tests from the test results tree,
in addition to installing the Kotest Intellij Plugin into your IDE, add the
[Kotest Gradle plugin](https://plugins.gradle.org/plugin/io.kotest) to your build.
...
![jump to source](./setup/jump_to_source.png)

As well as re-run all failed tests:

![re-run failed](./setup/rerun_failed.png)

Or re-run a single test from the results tree:

![re-run selected](./setup/rerun_selected_test.png)

## Re-running tests

By default, Gradle's incremental build will skip running tests if no source code has changed,
marking the task as UP-TO-DATE. This can be inconvenient during debugging.
