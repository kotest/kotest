...
The symbol processor added by the plugin uses the same Kotest version as the plugin itself. That version is available as `kotest.kotestVersion`, so other Kotest modules can use it instead of declaring the version again:
...
kotlin {
   sourceSets {
      commonTest {
         dependencies {
            implementation("io.kotest:kotest-assertions-core:${kotest.kotestVersion.get()}")
         }
      }
   }
}
...
:::tip
A working Android project with unit, instrumented and data driven test examples, can be found here:
https://github.com/kotest/kotest-examples
:::
...
For unit tests, Kotest uses the [JUnit Platform](https://junit.org/junit5/docs/current/user-guide/#running-tests-build-gradle) gradle plugin.
...
For instrumented tests, Kotest uses the [JUnit4 Runner](https://developer.android.com/training/testing/junit-runner).
