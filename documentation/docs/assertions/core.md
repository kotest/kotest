...
| `collection.shouldContainExactCopies(element, copies = N)` | Asserts that the collection has the specified element(s) in `collection`, specified N times, where N must be >=1.                                                                                       |
...
| `str.shouldContainExactCopies("substr", copies = N)` | Asserts that the string includes the given substring exactly N times, where N must be >=1.                                                                                                                       |
...
| `str.shouldContainInOrder(subtrings)`                | Asserts that the string contains all the given substrings in order, possibly with characters before, between, and after them. Substrings can overlap.                                                            |
| `str.shouldContainInOrderWithoutOverlaps(subtrings)` | Asserts that the string contains all the given substrings in order without overlaps, possibly with characters before, between, and after them.                                                                   |
